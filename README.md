...
GitHub Search API는 "현재 스타 수"만 줄 뿐 "증가 추세"는 주지 않는다. AgentRadar는 시점별 스냅샷을 ETL로 쌓아 델타를 계산함으로써 **"지금 뜨는 레포"**를 보여준다.
...
```
agent-radar/
├── docs/         # PRD · SPEC · API · DATA_MODEL · DIAGRAMS · DATA_PIPELINE · DESIGN
...
> [!TIP]
> 백엔드 없이 UI 전체를 데모하려면 `frontend/.env.local`에 `VITE_USE_MOCK=true`를 두고 `npm run dev` 하면 인메모리 목 데이터로 동작한다. 기본값은 실제 API 호출이다.

> [!NOTE]
> `GITHUB_TOKEN`은 공개 레포 검색만 하므로 **스코프 없이**(classic) 또는 "Public Repositories, read-only"(fine-grained)로 발급하면 된다. 토큰은 권한이 아니라 레이트리밋(5,000 req/hr)을 위해 쓴다.
...
> [!NOTE]
> `star_delta`/`growth_rate`는 `repos`에 **캐시 컬럼**으로 두어 목록 정렬·급상승 조회를 가속한다. (없으면 조회 시 스냅샷 2개를 비교해 계산도 가능)
...
> [!WARNING]
> **순서 주의** — 이번 스냅샷을 넣기 전에 직전 스냅샷을 먼저 읽어야 델타가 올바르게 계산된다. 순서가 뒤바뀌면 `star_delta`가 항상 0이 된다.
...
> [!IMPORTANT]
> **현재 구현 상태** — 백엔드 컨트롤러/모델/ETL은 아직 스텁(`// TODO`)이며, 프론트는 `VITE_USE_MOCK` 토글의 인메모리 목으로 완성돼 있다. `live` 전환은 백엔드 구현이 선행돼야 한다.
...
> [!TIP]
> [`db/schema.dbml`](db/schema.dbml)을 [dbdiagram.io](https://dbdiagram.io)에 붙여넣으면 ERD를 시각화/Export 할 수 있다.

---

## 다이어그램

> GitHub에서 바로 렌더된다. 전체 세트(아키텍처·유스케이스·컴포넌트 등)는 [docs/DIAGRAMS.md](docs/DIAGRAMS.md) 참고.
...
| [docs/SPEC.md](docs/SPEC.md) | 기술 명세 |
| [docs/API.md](docs/API.md) | REST API 계약 |
| [docs/DATA_MODEL.md](docs/DATA_MODEL.md) | 데이터 모델 |
| [docs/DATA_PIPELINE.md](docs/DATA_PIPELINE.md) | ETL 파이프라인 설계 + 테스트케이스 |
| [docs/DIAGRAMS.md](docs/DIAGRAMS.md) | 전체 다이어그램 (아키텍처·ERD·시퀀스·상태·플로우) |

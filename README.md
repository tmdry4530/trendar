...
> **GitHub 트렌드 레이더 (Trend + Radar)** — 사용자가 등록한 검색 조건에 매칭되는 GitHub 레포만 선별 수집하고, 시점별 스냅샷을 쌓아 **스타 증가율(추세)**을 계산하는 목적형 수집기.
...
├── backend/      # Node + Express — GitHub OAuth 인증 + 사용자별 ETL 파이프라인 + REST API
└── frontend/     # React + Vite + TypeScript 대시보드 (로그인 + 4 화면)
```
...
- **테넌트** : 수집은 **사용자 단위**로 실행 — 각 사용자의 활성 조건만, **그 사용자의 OAuth 토큰**으로 검색 (레이트리밋 각자 5,000 req/hr)
- **모드** : `query_type ∈ {topic, keyword}`
- **질의** : `topic` → `topic:<query>` / `keyword` → `<query> in:name,description,readme`
- **정렬 / 상한** : `sort=stars&order=desc`, `per_page = ETL_PER_QUERY`(기본 30), 조건은 사용자당 최대 `MAX_QUERIES_PER_USER`(기본 10)개
...
수동 트리거(`POST /api/etl/run`)와 cron 자동 동기화를 모두 지원한다. cron은 유효한 토큰을 가진 **사용자를 순회**하며 각자의 토큰으로 파이프라인을 실행하고, 멱등성(`(user_id, github_id)` upsert)·사용자/조건별 에러 격리·재실행을 보장한다. 같은 사용자의 중복 실행은 인메모리 락으로 409 처리된다.

![ETL 파이프라인](images/ETL%20파이프라인.png)

> 다이어그램은 **사용자 1명 기준의 내부 파이프라인**이다. v2에선 이 파이프라인이 사용자마다 각자의 OAuth 토큰으로 반복 실행된다.
>
> ⚠️ **순서 주의** — 이번 스냅샷을 넣기 전에 직전 스냅샷을 먼저 읽어야 델타가 올바르게 계산된다. 순서가 뒤바뀌면 `star_delta`가 항상 0이 된다.

### 1-8. 인증 · 멀티테넌시

- **로그인** — GitHub OAuth App (요청 스코프 없음 = 공개 데이터 최소 권한). `state` 파라미터로 콜백 위조(CSRF)를 방어하고, code 교환 → 프로필 upsert 후 access token을 **AES-256-GCM으로 암호화해 저장**한다 (크론 수집에 재사용).
- **세션** — JWT 대신 **DB 세션 + httpOnly 쿠키**(`sid`, SameSite=Lax, prod에서 secure). DB엔 토큰의 **sha256 해시만** 저장해 유출 시에도 하이재킹이 불가하며, 로그아웃·탈퇴 시 행 삭제로 즉시 무효화된다. 기본 30일(`SESSION_TTL_DAYS`).
- **격리** — `/api/health`·`/api/auth`를 제외한 모든 API가 인증 미들웨어 뒤에 있고, 모든 모델 쿼리가 `user_id` 스코프로 강제된다. 검색 조건·레포·북마크·메모·수집 상태 전부 사용자별.
- **토큰 무효 복구** — 사용자가 GitHub에서 연결을 revoke하면 수집 시 401로 감지 → `token_invalid` 마킹 + 토큰 파기 → 크론 대상 제외 + 화면에 재로그인 배너 → 재로그인 시 새 토큰으로 자동 복구.
- **탈퇴** — `users` 행 삭제 하나로 세션·조건·레포·스냅샷이 전부 `ON DELETE CASCADE` 연쇄 삭제.
...
| FR-06 | `(user_id, github_id)` 기준 멱등 upsert | 중복 행 방지 |
...
| FR-17 | GitHub OAuth 로그인 / 로그아웃 / 계정 탈퇴 | `/api/auth/*` |
| FR-18 | 모든 데이터 API 인증 필수 + 사용자별 데이터 격리 | 401 / `user_id` 스코프 |
| FR-19 | 사용자당 검색 조건 상한 | `MAX_QUERIES_PER_USER` (기본 10) |
| FR-20 | 사용자별 OAuth 토큰으로 수집 + 암호화 저장 | AES-256-GCM |
| FR-21 | 토큰 무효(revoke) 감지 및 재로그인 유도 | `token_invalid` + 배너 |
...
1. **레이트리밋 관리** — 사용자별 토큰(각 5,000 req/hr) + 조건 상한(10개)으로 구조적으로 분산됨. 남은 과제는 429(secondary limit) 백오프와 ETag 조건부 요청.
2. **스냅샷 보존 정책** — 무한 누적 → 다운샘플/아카이브 필요?
3. **파생 지표 위치** — 캐시 컬럼 vs 조회 시 계산.
4. **`recent` 정렬 기준** — `first_seen_at` vs `updated_at`?
5. **동시성** — 사용자별 인메모리 락으로 cron·수동 중복 실행은 409 처리됨. 남은 과제는 다중 인스턴스 배포 시 분산 락.
6. **크론의 사용자 순차 처리** — 사용자 수가 커지면 한 주기가 길어짐 → 동시성 제한 병렬화(p-limit)나 작업 큐로 확장.
...
| ✅ | 동일 `(user_id, github_id)` 재수집 시 UPDATE되어 repos 행이 늘지 않는다 |
| ✅ | 여러 조건이 같은 레포를 반환해도 사용자 내 중복 행이 없다 |
| ✅ | 서로 다른 사용자가 같은 레포를 watch하면 각자의 행으로 격리 저장된다 |
...
| ✅ | 같은 사용자의 `(query, query_type)` 중복 등록은 409 DUPLICATE로 거부된다 |
| ✅ | 사용자당 조건 상한(기본 10) 초과 등록은 400 QUERY_LIMIT_EXCEEDED로 거부된다 |
| ✅ | `is_active` 토글이 반영된다 |
...
| ✅ | 실행 후 사용자별 `last_etl_at/status/message`가 갱신된다 |
| ✅ | GitHub 빈 결과도 에러 없이 0건 처리된다 |
| ✅ | 토큰 401 시 해당 사용자만 `token_invalid`로 중단되고 다른 사용자 수집은 계속된다 |
...
| 분류 | 시나리오 |
| --- | --- |
| ✅ | 비로그인 상태의 데이터 API 호출은 401 UNAUTHORIZED를 반환한다 |
| ✅ | OAuth `state` 불일치 콜백은 로그인 실패로 처리된다 |
| ✅ | 만료된 세션 쿠키는 401 처리되고 해당 세션 행이 정리된다 |
| ✅ | 세션 만료 시 프론트가 즉시 로그인 화면으로 전환한다 (401 공통 처리) |
| ✅ | 계정 탈퇴 시 세션·조건·레포·스냅샷이 연쇄 삭제된다 |
| ❌ | 다른 사용자의 데이터가 목록·통계·트렌드에 섞이면 안 된다 |
| ❌ | 세션 토큰 원문이 DB에 저장되면 안 된다 (sha256 해시만) |

---

## 5. ERD 설계

관계: `users 1 ──< sessions` · `users 1 ──< watch_queries 1 ──< repos 1 ──< repo_snapshots` (모두 `ON DELETE CASCADE` — 탈퇴 시 전체 연쇄 삭제).
...
erDiagram
    ...
    users {
        ...
        bigint github_id UK
        varchar login
        text access_token_enc "AES-256-GCM 암호문"
        boolean token_invalid
        datetime last_etl_at
    }
    sessions {
        int id PK
        int user_id FK
...
- **`users`** (테넌트) — GitHub 프로필 + 암호화 토큰 + 수집 상태. 모든 데이터의 루트.
- **`sessions`** (인증) — 세션 토큰의 sha256 해시와 만료 시각. 로그아웃·탈퇴 시 행 삭제 = 즉시 무효화.
- **`watch_queries`** (도메인) — 추적 검색 조건 마스터. `UNIQUE(user_id, query, query_type)`.
- **`repos`** (도메인) — 수집 레포 마스터. `(user_id, github_id)` 멱등 upsert + 파생 캐시(`stars`/`star_delta`/`growth_rate`) + 북마크·메모.
...
> 아래 다이어그램은 **수집 파이프라인(사용자 1명 기준의 내부 루프)**을 그린 것이다. v2 멀티테넌트에선 이 파이프라인이 유효한 토큰을 가진 사용자마다 각자의 OAuth 토큰으로 반복 실행된다 (§1-7).

...
export async function upsertRepo(repo) {
  const [res] = await pool.query(
    `INSERT INTO repos (github_id, query_id, full_name, owner, name, html_url, description, language, stars)
     VALUES (?,?,?,?,?,?,?,?,?)
     ON DUPLICATE KEY UPDATE
       ...
       language=VALUES(language), stars=VALUES(stars)`,
    [repo.github_id, repo.query_id, repo.full_name, repo.owner, repo.name,
     repo.html_url, repo.description, repo.language, repo.stars]
  );
  if (res.insertId) return res.insertId;
  const [[row]] = await pool.query('SELECT id FROM repos WHERE github_id=?', [repo.github_id]);
  return row.id;
}

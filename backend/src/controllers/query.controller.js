...
export async function list(req, res, next) {
  try { res.json({ ok: true, data: await Query.findAll() }); }
  catch (e) { next(e); }
}
...
export async function create(req, res, next) {
  try {
    ...
    res.status(201).json({ ok: true, data: await Query.create({ query: query.trim(), query_type }) });
  } catch (e) {
...
export async function update(req, res, next) {
  try {
    const data = await Query.update(Number(req.params.id), req.body ?? {});
...
export async function remove(req, res, next) {
  try {
    if (!await Query.remove(Number(req.params.id))) throw httpError(404, 'NOT_FOUND', '조건을 찾을 수 없습니다.');
